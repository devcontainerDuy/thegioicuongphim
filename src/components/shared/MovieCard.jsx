...
const MovieCard = ({ name, slug, image, totalEpisodes, currentEpisode, time, quality = "HD", averageScore = 0, totalRatings = 0, loading = false, className }) => {
    if (loading) {
        return (
    ...
    return (
        <ClickSpark sparkCount={8} sparkColor="hsl(var(--primary))">
            <motion.div whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.3, ease: [0.25, 0.4, 0.25, 1] }} className={cn("group relative rounded-lg cursor-pointer", className)}>
                <Link to={`/phim/${slug}`}>
                    <div className="relative aspect-[2/3] overflow-hidden rounded-lg bg-muted dark:bg-zinc-800 border border-transparent dark:border-zinc-800 shadow-sm transition-all hover:shadow-md">
                        {image ? (
                            <img src={image} alt={name} className="h-full w-full object-cover transition-opacity duration-300 group-hover:opacity-80" loading="lazy" />
                        ) : (
                    ...
                    <div className="mt-2 space-y-1">
                        <h3 className="font-bold text-sm line-clamp-1 text-foreground group-hover:text-primary transition-colors">{name}</h3>
                        <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">{time && <span>{time}</span>}</div>
                    </div>
                </Link>
            </motion.div>
        </ClickSpark>
