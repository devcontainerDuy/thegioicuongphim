 ...
 */
export const SavedMoviesLayout = ({
  title,
  icon,
  items = [],
  emptyMessage,
      ...
      <div className="container mx-auto px-4 md:px-12">
        ...
        {isLoading ? (
                  ...
                  <div className="flex-1 py-1 flex flex-col justify-between min-w-0">
                    <div>
                      <div className="flex items-start justify-between gap-2 sm:gap-4">
                        <Link
                          to={`/phim/${film.slug}`}
                          className="text-base sm:text-xl font-bold text-white group-hover:text-primary transition-colors line-clamp-1"
                        >
                          {film.name}
                        </Link>
                      ...
                      <p className="text-zinc-500 text-xs sm:text-sm mt-1 line-clamp-1">
                        {film.original_name || film.originalName}
                      </p>
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-20 bg-zinc-900/30 rounded-3xl border border-zinc-800 border-dashed px-4">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-zinc-800 rounded-full flex items-center justify-center mb-6">
              <div className="w-8 h-8 sm:w-10 sm:h-10 text-zinc-600">{icon}</div>
            </div>
            <h3 className="text-lg sm:text-xl font-bold text-white mb-2">{emptyMessage}</h3>
            <p className="text-zinc-500 mb-6 sm:mb-8 text-center max-w-md text-sm sm:text-base px-4">
              {emptyDescription}
            </p>
