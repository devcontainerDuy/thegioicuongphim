...
import { Filter, ChevronRight, Loader2, Grid, List, Play } from "lucide-react";
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                    ...
                    {sidebar && (
                        <div className="lg:hidden w-full mb-4">
                            <Button variant="outline" className="w-full flex items-center gap-2 justify-between" onClick={onToggleSidebar}>
                    ...
                    {/* Sidebar / Filter */}
                    {sidebar && <aside className={`w-full lg:w-64 lg:sticky lg:top-24 shrink-0 space-y-6 ${showMobileSidebar ? "block" : "hidden lg:block"}`}>{sidebar}</aside>}
                    ...
                    <main className="flex-1 w-full min-w-0">
                        ...
                        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 pb-6 border-b border-border dark:border-zinc-800">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                {/* Card View Toggle */}
                                <div className="flex items-center gap-2">
                                    <span className="text-sm text-muted-foreground hidden sm:block">Xem dạng:</span>
                                    <Tabs value={cardView} onValueChange={onCardViewChange} className="w-fit">
                                        <TabsList className="bg-card dark:bg-zinc-900 border border-border dark:border-zinc-700">
                                            <TabsTrigger value="grid" title="Xem dạng lưới">
                                                <Grid className="h-4 w-4" />
                                            </TabsTrigger>
                                            <TabsTrigger value="list" title="Xem dạng danh sách">
                        ...
                        {/* Movie Grid */}
                        <div className={cardView === "grid" ? "grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-4 gap-y-8" : "space-y-4"}>
                                ...
                                if (cardView === "grid") {
                                    return (
                                        <div ref={ref} key={`${movie.slug}-${index}-grid`} className="w-full">
                                            <MovieCard name={movie.name} slug={movie.slug} image={movie.thumb_url} totalEpisodes={movie.episode_current} currentEpisode={movie.current_episode} time={movie.time} quality={movie.quality} />
                                        </div>
                                    );
                                } else {
                                    return (
                        ...
                        {viewMode === "infinite" && !loading && !error && items.length > 0 && currentPage >= totalPage && <div className="text-center py-12 text-muted-foreground text-sm italic">Bạn đã xem hết danh sách.</div>}
...
const ListItem = ({ movie }) => (
    <motion.div
        ...
        transition={{ delay: movie.id * 0.05 }}
        key={movie.id}
    ...
    >
        <Link to={`/phim/${movie.slug}`} className="w-20 sm:w-24 h-28 sm:h-36 rounded-xl overflow-hidden shrink-0 border border-zinc-800">
            <img src={movie.thumb_url || movie.poster_url} alt={movie.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
        </Link>
        <div className="flex-1 py-1 flex flex-col justify-between min-w-0">
            <div>
                <div className="flex items-start justify-between gap-2 sm:gap-4">
                    <Link to={`/phim/${movie.slug}`} className="text-base sm:text-xl font-bold text-foreground group-hover:text-primary transition-colors line-clamp-1">
                        {movie.name}
                    </Link>
                ...
                <p className="text-muted-foreground text-xs sm:text-sm mt-1 line-clamp-1">{movie.original_name || movie.originalName}</p>
