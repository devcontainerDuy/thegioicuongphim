            ...
            <Container className="home-page pt-5">
                ...
                <section className="home-info-banner">
                    <div>
                        ...
                        <p className="text-body-secondary mb-0">Chúng tôi chỉ giữ những khối nội dung quan trọng – spotlight, danh sách thịnh hành và vài kệ phim chọn lọc. Bạn không phải cuộn qua quá nhiều banner hay khối quảng bá nữa.</p>
                    </div>
                ...
                <FilmRailSection title="Đang thịnh hành hôm nay" films={trendingList} viewAllLink="/danh-sach-phim" loading={loading && !trendingList.length} />

                {sectionResults.map((config) => (
                    <FilmGridSection key={config.key} title={config.title} films={sectionsData[config.key] || []} viewAllLink={config.viewAll} loading={loading && !sectionsData[config.key]?.length} />
                ))}
            </Container>
        </Template>
    );
