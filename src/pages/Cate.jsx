                ...
                {categories.map((cat) => {
                    ...
                    return (
                        <div key={cat.slug} className="space-y-2">
                            <button
                                onClick={() => (cat?.item?.length > 0 ? handleSubClick(cat.slug, cat.item?.[0]?.slug) : handleCategoryClick(cat.slug))}
                                className={cn(
                            ...
                            >
                                {cat.name} {cat?.item?.length > 0 ? `(${cat?.item?.length})` : ""}
                                {cat?.item?.length > 0 && <ChevronRight className={cn("w-4 h-4 transition-transform duration-200", isActive && "rotate-90")} />}
                            </button>
                                    ...
                                    {cat.item.map((subItem) => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                subParam === subItem.slug ? "text-primary font-bold bg-muted dark:bg-zinc-800" : "text-muted-foreground hover:text-foreground",
                                            )}
                                        >
                                            {subItem.name}
                                        </button>
